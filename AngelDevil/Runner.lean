...
-- TODO: Consider using a different name than 'step' since "steps" already
-- refers to the cells in a journey
/- Add one step to the run.
   If the step would exceed the maximum allowed steps per sprint, or is not "close"
   to the starting point of the sprint, then instead end the sprint.
   If the sprint doesn't end, then try taking the next step.
-/
def step (p ns : Nat) (start : Int × Int) (eaten : List (Int × Int)) (rs : RunState) : List RunState :=
  if hterm : 2 * p < ns ∨ ¬close p start (loc rs) then [] else
  rs :: (step p (ns + 1) start eaten (next_step eaten rs))
...
-- If the list constructed by 'step' is not empty, the first element of the list will be 'rs'
lemma step_getElem_zero_of_nonnil (p ns : Nat) (start : Int × Int) (eaten : List (Int × Int)) (rs : RunState) :
  (hnnil : step p ns start eaten rs ≠ []) → (step p ns start eaten rs)[0]'(List.length_pos_iff.mpr hnnil) = rs := by
  intro hnnil
  -- Use the trick of starting with 'step = step' followed by 'nth_rw'
  ...
  have : step p ns start eaten rs = _ := rfl
  nth_rw 2 [step] at this
...
-- If the list constructed by 'step' is not empty, ns < 2 * p + 1
lemma step_nslt_of_nonnil (p ns : Nat) (start : Int × Int) (eaten : List (Int × Int)) (rs : RunState) :
  (hnnil : step p ns start eaten rs ≠ []) → ns < 2 * p + 1 := by
  intro hnnil
...
-- If the list constructed by 'step' is not empty, then the first element is close to 'start'
lemma step_close_getElem_zero_of_nonnil (p ns : Nat) (start : Int × Int) (eaten : List (Int × Int)) (rs : RunState) :
  (hnnil : step p ns start eaten rs ≠ []) →
  close p start (loc ((step p ns start eaten rs)[0]'(List.length_pos_iff.mpr hnnil))) := by
  intro hnnil
  rw [step_getElem_zero_of_nonnil]
  · contrapose! hnnil
...
-- If the list constructed by 'step' is not empty, rewrite it in 'cons' form.
lemma step_cons_of_nonnil (p ns : Nat) (start : Int × Int) (eaten : List (Int × Int)) (rs : RunState) :
  (hnnil : step p ns start eaten rs ≠ []) →
  step p ns start eaten rs = (rs :: (step p (ns + 1) start eaten (next_step eaten rs))) := by
  intro hnnil
  nth_rw 1 [step]
...
lemma step_mem_close (p ns : Nat) (start : Int × Int) (eaten : List (Int × Int)) (rs : RunState) :
  ∀ x ∈ step p ns start eaten rs, close p start (loc x) := by
  intro x hxmem
  rcases List.getElem_of_mem hxmem with ⟨i, ilt, helem⟩
  by_cases iz : i = 0
  · -- We've already proven that if 'step ≠ []', step[0] is "close"
    subst iz
    subst helem
    apply step_close_getElem_zero_of_nonnil
  ...
  · rename' iz => inz; push_neg at inz
    ...
    rw [getElem_congr_coll (step_cons_of_nonnil p ns start eaten rs (List.ne_nil_of_mem hxmem))] at helem
    rw [getElem_congr_idx ((Nat.sub_one_add_one inz).symm), List.getElem_cons_succ] at helem
    exact step_mem_close _ _ _ _ _ x (List.mem_of_getElem helem)
termination_by (2 * p + 1 - ns)
decreasing_by
  have hnnil : step p ns start eaten rs ≠ [] :=
    List.ne_nil_of_length_pos (lt_of_le_of_lt (Nat.zero_le _) ilt)
  have nsle : ns ≤ 2 * p := by
    apply Nat.le_of_lt_add_one
    apply step_nslt_of_nonnil
    exact hnnil
...
lemma sprint_close_first_last (p : Nat) (hpos : 0 < p) (rs₀ : RunState) (eaten : List (Int × Int)) :
  ...
  unfold sprint at *
  rw [List.head_eq_getElem_zero hnnil, step_getElem_zero_of_nonnil]
  · exact step_mem_close p 1 (loc rs₀) eaten _ _ (List.getLast_mem hnnil)
  · assumption
