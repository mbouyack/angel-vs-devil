...
def segment_start (seg : TraceSegment) : RunState :=
  (segment_trace seg)[seg.i]'(lt_trans seg.ilt seg.jlt)
...
def segment_split_first (seg : TraceSegment) (k : Nat) (ltk : seg.i < k) (klt : k < seg.j) : TraceSegment where
  n := seg.n
  start := seg.start
  blocked := seg.blocked
  i := seg.i
  j := k
  ilt := ltk
  jlt := lt_trans klt seg.jlt

-- Split the segment into two pieces and take the second part
def segment_split_second (seg : TraceSegment) (k : Nat) (_ : seg.i < k) (klt : k < seg.j) : TraceSegment where
  n := seg.n
  start := seg.start
  blocked := seg.blocked
  i := k
...
lemma segment_states_length (seg : TraceSegment) :
  ...
  apply Nat.add_one_le_of_lt (Nat.sub_lt_sub_right (le_of_lt seg.ilt) _)
  apply (trace_length _ _ _) ▸ seg.jlt

-- Get the length of the first half of a split trace
lemma segment_split_first_length (seg : TraceSegment) (k : Nat) (ltk : seg.i < k) (klt : k < seg.j) :
  segment_length (segment_split_first seg k ltk klt) = k - seg.i + 1 := by
  unfold segment_length segment_split_first; simp

-- Get the length of the second half of a split trace
lemma segment_split_second_length (seg : TraceSegment) (k : Nat) (ltk : seg.i < k) (klt : k < seg.j) :
  segment_length (segment_split_second seg k ltk klt) = seg.j - k + 1 := by
...
lemma segment_split_length (seg : TraceSegment) (k : Nat) (ltk : seg.i < k) (klt : k < seg.j) :
  segment_length seg =
  (segment_length (segment_split_first seg k ltk klt)) +
  (segment_length (segment_split_second seg k ltk klt)) - 1 := by
  rw [segment_split_first_length, segment_split_second_length, segment_length]; simp
  rw [add_comm _ (seg.j - k), ← add_assoc, ← Nat.add_sub_assoc (le_of_lt ltk)]
  rw [Nat.sub_add_cancel (le_of_lt klt)]

-- Get an element from the first half of a split trace segment
lemma segment_split_first_states_getElem
  (seg : TraceSegment) (k : Nat) (ltk : seg.i < k) (klt : k < seg.j) :
  ∀ a (alt : a < (segment_states (segment_split_first seg k ltk klt)).length),
  (segment_states (segment_split_first seg k ltk klt))[a] =
  (segment_states seg)[a]'(by
    rw [segment_states_length, segment_split_first_length] at alt
    rw [segment_states_length, segment_length]
    exact lt_trans alt ((Nat.add_lt_add_right (Nat.sub_lt_sub_right (le_of_lt ltk) klt)) _)
  ) := by
  intro _ _
...
lemma segment_split_second_states_getElem
  (seg : TraceSegment) (k : Nat) (ltk : seg.i < k) (klt : k < seg.j) :
  ∀ a (alt : a < (segment_states (segment_split_second seg k ltk klt)).length),
  (segment_states (segment_split_second seg k ltk klt))[a] =
  (segment_states seg)[a + k - seg.i]'(by
    rw [segment_states_length, segment_split_second_length] at alt
    rw [segment_states_length, segment_length]
    apply Nat.sub_lt_right_of_lt_add (le_of_lt (lt_of_lt_of_le ltk (Nat.le_add_left _ _)))
    rw [add_right_comm, Nat.sub_add_cancel]; swap
    · exact le_of_lt (lt_trans ltk klt)
  ...
  exact le_of_lt (lt_of_lt_of_le ltk (Nat.le_add_left _ _))

-- Prove an identity for the states of the first half of a split segment
lemma segment_split_first_states (seg : TraceSegment) (k : Nat) (ltk : seg.i < k) (klt : k < seg.j) :
  segment_states (segment_split_first seg k ltk klt) =
  List.take (k - seg.i + 1) (segment_states seg) := by
  apply List.ext_getElem
  · rw [segment_states_length, segment_split_first_length]
    rw [List.length_take_of_le]
    rw [segment_states_length, segment_length]
    exact Nat.add_le_add_right (Nat.sub_le_sub_right (le_of_lt klt) _) _
  intro a alt alt'
...
lemma segment_split_second_states (seg : TraceSegment) (k : Nat) (ltk : seg.i < k) (klt : k < seg.j) :
  segment_states (segment_split_second seg k ltk klt) =
  List.drop (k - seg.i) (segment_states seg) := by
  apply List.ext_getElem
  · rw [segment_states_length, segment_split_second_length, List.length_drop]
    rw [segment_states_length, segment_length]
    rw [Nat.sub_sub_right _ (le_of_lt ltk), add_right_comm _ 1]
    rw [Nat.sub_add_cancel (le_of_lt (lt_trans ltk klt))]
    rw [Nat.sub_add_comm]
    exact le_of_lt klt
  intro a alt _
...
lemma segment_split_states (seg : TraceSegment) (k : Nat) (ltk : seg.i < k) (klt : k < seg.j) :
  segment_states seg =
  segment_states (segment_split_first seg k ltk klt) ++
  (segment_states (segment_split_second seg k ltk klt)).tail := by
  apply List.ext_getElem
  · rw [List.length_append, List.length_tail, ← Nat.add_sub_assoc]; swap
    · rw [segment_states_length]
      unfold segment_length segment_split_second; simp
    repeat rw [segment_states_length]
    exact segment_split_length seg k ltk klt
  intro a alt _
  by_cases alt' : a < (segment_states (segment_split_first seg k ltk klt)).length
  ...
  · rename' alt' => lea; push_neg at lea
    ...
    · rw [add_comm]
      exact le_of_lt (lt_trans ltk (Nat.lt_add_one _))
    rw [Nat.sub_add_comm (le_of_lt ltk), Nat.sub_add_cancel]
